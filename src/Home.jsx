...
const Home = () => {
  ...
  const PrivateTeacherRoute = () => {
    return localStorage.getItem("role") === "Teacher" ? <Outlet /> : <Navigate replace to="/login" />;
  };

  // Private route for Heads
  const PrivateHeadRoute = () => {
    return localStorage.getItem("isUser") === "true" &&
      (localStorage.getItem("role") === "Head" || localStorage.getItem("role") === "Management") ? (
      <Outlet />
    ) : (
  ...
  const PrivateStudentRoute = () => {
    return localStorage.getItem("role") === "Student" ? <Outlet /> : <Navigate replace to="/login" />;
  };

  // Public route accessible only if not logged in
  const PublicRoute = () => {
    return localStorage.getItem("isUser") === "true" ? <Navigate replace to="/" /> : <Outlet />;
  };
